...
### 4. Grant After Effects write permission (one-time)

The pipeline drives After Effects headlessly via `nexrender`, which patches AE's `commandLineRenderer.jsx` the first time it runs. That patch lives inside the AE install folder, which is OS-protected — so you need to grant write access **once per machine**. After this, every pipeline run works without admin/sudo.
...
If you skip this step, Step 4 of the pipeline will fail with `EACCES: permission denied` (macOS) or `EPERM` (Windows) when nexrender tries to back up the original AE script.
